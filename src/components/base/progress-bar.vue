...
<script>
    import { prefixStyle } from "../../common/js/dom"; //用来处理样式兼容的自定义前缀的
    ...
    export default {
        ...
        methods: {
            progressTouchStart(e){
                ...
                this.touch.left = this.$refs.progress.clientWidth;//获取当前进度条走过的地方的元素的宽度
            },
            progressTouchMove(e) {
                //如果没有点中那个按钮就不要激活滑动事件
        ...
        watch: {
            percent(newPercent) {
                if (newPercent >= 0 && !this.touch.initiated) {//当比例值发生变化时, 我们重新设置一次进度条的效果
